...
description: >
  ...
  "生成PPT", "做PPT", "制作演示文稿", or mentions "ppt-creator".
---
...
>
> - `ppt-creator` is a repository-specific workflow, not a general application scaffold
...
| `live-preview` | `workflows/live-preview.md` | Browser-based live preview — auto-started during generation and re-enterable any time the user mentions "live preview", "preview", "看效果", wants to click/select a slide element, **or asks for ANY edit — however small — to an already-generated project's `svg_output/*.svg`** |
...
> **Existing-project SVG edits — always preview first**: whenever the user asks for ANY change to an already-generated ppt-creator project's `svg_output/*.svg` — a full style pass or a single-word tweak on one page, it makes no difference — signal live preview activation first (`workflows/live-preview.md` Step 1, if not already signaled this session) **before** editing the SVG, so the user watches every change land live. Skipping this makes a correctly-applied edit invisible: the 「预览幻灯片」tab is always present, but it stays on its empty state until preview_open.py has run at least once this session. Route color/font-wide changes through `${SKILL_DIR}/scripts/update_spec.py` when the change is a `spec_lock`-level palette/font swap propagated across every page; use direct SVG edits for anything finer-grained — either way, the preview must already be signaled first.
...
- **cwd (or any parent segment) starts with `.`** → pass `--dir "$HOME/ppt-creator-projects"`.
...
| `kind: deck` | `design_spec.md` + template SVGs → `<project>/templates/`; logos / backgrounds / other **bitmaps** → `<project>/images/`. Strategist locks all segments; Eight Confirmations narrows to deck-content fields (audience / page count / outline / tone tweaks). The narrowing is mechanical, not just editorial: name the settled anchors in the Confirm UI's `locked` block (Step 4.1) so the page shows them read-only instead of offering a picker the user's answer can't change. |
...
> ⛔ **Steps 2 → 3 → 4 are ONE uninterrupted run — do NOT yield to the user mid-flow.** When the tier-1 wait (step 2) returns, the AI **immediately and autonomously** continues to step 3 (re-derive + write Tier 2) and step 4 (final wait) in the **same turn**: do **not** summarize, ask a question, report progress, or end the turn in between. The 「问题」canvas tab is sitting on a "deriving…" spinner polling for the Tier 2 you must write next — stopping here strands the page and the user must prod you in chat to finish (a bug, not the intended flow). **The tier-1 confirmation is an intermediate machine handoff, not a stopping point.** The single ⛔ BLOCKING wait is the **final** confirmation at the end of step 4. (Chat-fallback path — only when the app never rendered the tab — is the exception: there you do present each tier in chat and wait for a reply.)
...
1. **Write Tier 1** to `<project_path>/confirm_ui/recommendations.json` with `"tier": 1` and only the anchor fields. Enumerable anchors (`canvas` / `mode` / `visual_style` / `delivery_purpose`) name a recommended canonical `id` in a `recommend` block (the page lists common options from `confirm_ui/static/catalogs.json`); `visual_style` also carries the ≥3-style `visual_style_spectrum` (safe / shifted / bold — same hard rule as h.5). `audience` and `content_divergence` are plain `{ "value": "<free text>" }`. `content_divergence` is the **free-text** field shown under audience in §c — how closely to follow the source vs how freely to reshape it (blank = balanced; facts stay sourced at every level); it is consumed by Strategist when authoring `§IX`, recorded in `design_spec.md §I`, carries no page-count coupling, and is **not** written to `spec_lock.md`. Set `lang` to the page language; visible text matches `lang`, or provide bilingual `name_zh` / `name_en` + `note_zh` / `note_en`.
   > **Template-locked anchors (`kind: deck`).** When Step 3 dispatched a deck template (alone or in a fusion), the anchors that template already fixes are settled facts, not questions — this is what the kind matrix's "Eight Confirmations narrows to deck-content fields" means in practice. Name them in a top-level `locked` block with their source and the page renders each as a **read-only row** instead of a chip picker:
   > ```json
   > "locked": { "canvas": "招商银行模板", "visual_style": "招商银行模板" }
   > ```
   > Lock only what the deck genuinely constrains — `canvas` and `visual_style` always; `mode` / `delivery_purpose` only when the deck's `design_spec.md` actually pins them. Where the values come from: `canvas` is the spec's `canvas_format`, verbatim. `visual_style` is **your** reading of the spec's design tone / visual DNA mapped onto a `visual_styles` catalog id — deck specs do not carry that id, so state the mapping and its evidence in `notes.visual_style` (e.g. "editorial —— 品牌红头条 + 浅色内容页 + 卡片网格 + ≤60% 内容占比"). A locked `visual_style` skips `visual_style_spectrum` entirely: a spectrum offers choice, and this field no longer has any. **Keep writing each locked field's `recommend.*` value** — that value is what the row displays and what lands in `result.json`; omitting it does not hide the field, it just throws the template's own value away for a catalog default. `kind: brand` / `kind: layout` lock nothing in Tier 1 (brand's identity surfaces in Tier 2; layout constrains structure, not anchors), and the free-design path writes no `locked` block at all — there, every anchor is a real choice.
2. **Wait for Tier 1.** One command — no server to launch, no port to read back. `--fresh` starts a new confirmation round (clears any stale prior result, snapshots the catalog the page will show). **Long tool timeout — 600000 ms** (the ≈590 s budget):
   ```bash
   python3 ${SKILL_DIR}/scripts/confirm_ui/confirm_wait.py <project_path> --stage tier1 --fresh
   ```
   The Claude Desktop host detects this command and renders the Eight-Confirmations page NATIVELY in the 「问题」canvas tab (CanvasConfirm) — no browser, no port, nothing to embed. The tab does **not** close after Tier 1: it shows a "deriving…" state and polls for Tier 2. **Wait failure is non-fatal**: if it times out (no desktop host, remote / web / headless environment), do **NOT** troubleshoot — **on any non-zero exit, re-check `result.json` once** (a fresh `status: tier1-confirmed`) before dropping to the chat fallback. **On success (exit 0 with a tier-1 result), do not pause or report — go straight to step 3 in the same turn.** **Never pass `--timeout` and never re-run this exact wait a second time** — a timeout means move on to the chat fallback, not "wait again"; the script's own budget is a fixed, non-negotiable ceiling (any `--timeout` value is clamped server-side), so retrying only burns another full budget for the same non-result and leaves the user staring at a turn that never seems to finish.
...
4. **Wait for the final confirmation** — no `--fresh` (it would delete the tier-1 result this round depends on). Same 600000 ms budget:
   ```bash
   python3 ${SKILL_DIR}/scripts/confirm_ui/confirm_wait.py <project_path> --stage final
   ```
   This is the ⛔ BLOCKING completion: returns when the page writes the final `result.json` (`status: confirmed`, `stage: final`, carrying all Tier 1 + Tier 2 fields). On a non-zero exit, re-check `result.json` once, then drop to the chat fallback — **never pass `--timeout` and never re-run this wait a second time** (same rule as step 2; the budget is fixed and non-negotiable, so retrying just repeats the same non-result for another full wait). Confirmed sizes are **already px** (the system is px-only — no pt anywhere, no conversion): write `result.json` `typography.body_size` / `sizes` into `design_spec.md` / `spec_lock.md` / SVG verbatim. `generation_mode: "split"` / `refine_spec: true` are explicit user choices. **No cleanup step**: there is no server to shut down — the confirm tab's data is just files, and the tab clears itself once this wait returns with a final result (see usePreviewServer in the app).

**Always also print each tier's recommendations in chat** as the always-valid fallback. **The chat fallback is two-step too**: if the tab never appears or a wait times out with no fresh result, present Tier 1 in chat → get confirmation → re-derive → present Tier 2 → get confirmation → take those values. Either path converges. (Non-desktop / headless environments: the pre-refactor `server.py` browser fallback has been retired along with the Flask surface it ran — there is no bundled non-desktop UI for this step anymore; the chat fallback is the only path outside Claude Desktop.)
...
**Narrow exception — suspected-misclick vs. the user's own stated wording (not vs. your recommendation).** The default above (silently reconcile, narrate afterward) is for the ordinary case: the user deliberately picked something other than what *you* recommended — that is always their call, never a reason to stop. Stop and ask **only** when the changed anchor **literally contradicts a concrete style/anchor word the user themselves typed earlier in this same conversation** (not merely "different from your recommendation") — e.g. the user wrote "科技感要强" / "tech-forward, strong" and Tier 1 came back `memphis` (festival/retro/consumer — the opposite direction), or the user said "极简" and Tier 1 came back a maximalist/ornate pick. This is not a "confirmed value wins" override to second-guess; it is a **misclick signal**: UI multi-select controls make it easy to land on an adjacent option, and honoring it silently risks building the rest of the deck on a choice the user never actually made. When (and only when) this exact condition is met: continue the same turn (do not end it) and ask **one short, single-question** confirmation before step 3 — e.g. "你在 Tier 1 选了 `memphis`，但这和你说的'科技感要强'方向相反，是有意换风格，还是点岔了？" — then proceed with whichever answer they give. Do not generalize this into a broader license to pause on any anchor change; the table above and the no-blocking-wait rule remain the default for everything else, including cases where the pick merely diverges from *your* recommendation.
...
**Live Preview Auto-Startup (Mandatory)**: before the first SVG, signal that live preview should activate, so the Claude Desktop host lights up the 「预览幻灯片」canvas tab and starts reading `svg_output/` directly (no server, no port):
```bash
python3 ${SKILL_DIR}/scripts/svg_editor/preview_open.py <project_path> --live
```
- Start it immediately when Executor begins; `svg_output/` may be empty (`--live` creates the directory if needed). This command resolves the project's absolute path itself and prints `live preview ready: <path>` to stdout, then exits at once — it is a one-shot activation signal, not a long-running process. The Claude Desktop host detects the command and its printed path and renders `svg_output/` NATIVELY in the 「预览幻灯片」canvas tab (LivePreviewEditor).
- Nothing to keep running: generation continues writing/rewriting `svg_output/*.svg` and the app's own polling picks up every change — there is no side process to babysit or explicitly stop. The tab simply reflects whatever `svg_output/` currently contains, for the rest of the session.
...
- The editor supports **annotations** (select an element → attach an edit instruction → 应用我的标注 sends the whole batch to chat for the AI to act on) — this is the only edit surface; there is no direct attribute/text editing or undo (never carried over from the retired browser editor). Full scope: see [`workflows/live-preview.md`](workflows/live-preview.md) Notes.
...
- [x] Live preview activated (preview_open.py) and 「预览幻灯片」reflecting svg_output/
...
> **Post-export annotation window**: 「预览幻灯片」stays populated after export — it's just a live read of `svg_output/`, nothing to keep running or fall over. If the user annotated slides (during Executor or after export) and now asks to apply them — they may quote the composer message the app sends on their behalf (`应用我的标注`), or say "apply my annotations" / "应用注解" / equivalent — run [`live-preview`](workflows/live-preview.md) Step 2 to apply and re-export. Annotations submitted during generation are also handled here, not earlier.

> **Preview tab empty?** Any time the user mentions "live preview", "preview", "看效果", or wants to select/click a slide element and the 「预览幻灯片」tab isn't showing anything, run [`live-preview`](workflows/live-preview.md) Step 1 to (re-)signal activation. There's no running/not-running service state to check — the tab just reflects whatever `svg_output/` currently contains.
