...
Resolve `<resolved_dir>` the same way as SKILL.md Step 2 (`pwd`; `$HOME/ppt-creator-projects` if cwd is dotfile-prefixed, else `$(pwd)/projects`) — never the bare command's implicit default.
...
Write `<project_path>/confirm_ui/recommendations.json` and wait for it via the same `confirm_wait.py` SKILL.md Step 4 uses. Do **not** hide fields: seed **every** targeted-confirmation field with the inherited / source-derived default so the user sees the recommendation and keeps the place to change it. Schema → [`scripts/docs/confirm_ui.md`](../scripts/docs/confirm_ui.md).
...
Wait for the (single-pass) confirmation — `--fresh` starts the round, `--stage final` matches beautify writing straight to `stage: final` (no tier-1 leg):

```bash
python3 ${SKILL_DIR}/scripts/confirm_ui/confirm_wait.py <project_path> --stage final --fresh
```

The Claude Desktop host detects this command and renders the Eight-Confirmations page NATIVELY in the 「问题」canvas tab (CanvasConfirm) — no server, no port, no browser to embed.

Read the confirmed canvas + palette + typography (incl. `body_size`) and any other overrides from `<project_path>/confirm_ui/result.json`. Chat is the canonical fallback when the tab never renders (remote / headless / non-desktop) — present the same fields in chat and honor the reply identically. No cleanup step needed — there is no server to shut down.
