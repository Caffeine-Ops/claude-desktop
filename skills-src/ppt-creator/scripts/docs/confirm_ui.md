...
|---|---|---|---|
| `1` | anchors — canvas, audience + `content_divergence` + `delivery_purpose` *(PPT only — omitted on non-PPT canvases, not written to the result)* (all in the §c key-info area), mode + visual_style — any of these four enumerables renders **read-only** when listed in `locked` (see below) | **Next** | writes `result.json` `{ stage: "tier1", status: "tier1-confirmed", <anchors> }`; the page does **not** close — it shows a "deriving…" state and polls `recommendations.json` via CONFIRM_UI_READ |
...
- **`locked` — the anchors a template already settled** (optional, Tier 1 only). When the user supplied a `kind: deck` template, `canvas` and `visual_style` (often `mode` / `delivery_purpose` too) are decided facts, not open questions — SKILL.md's deck dispatch requires the Eight Confirmations to *narrow to deck-content fields*. Declare them in a top-level `locked` block and the page renders each as a **read-only row** (value + lock icon + source) instead of a chip picker:
  ...
  A string (or a `{ "zh", "en" }` pair) is displayed as the source; `true` locks without naming one. Only the four Tier-1 enumerable anchors are lockable — `canvas` / `mode` / `visual_style` / `delivery_purpose`. Free-text `audience` / `content_divergence` are deck-content fields and always stay editable, as does everything in Tier 2 (a deck template fixes the identity, not the page count or the image plan).

  **Beautify / template-fill never lock** — those workflows inherit the source deck's identity as a *default*, not as a constraint, and their own rule is explicit: seed every field so the user keeps the place to change it (see [`beautify-pptx.md`](../../workflows/beautify-pptx.md)). `locked` is for a template the user chose *as* the design contract.

  **A locked field still needs its `recommend.*` value** — that value is what gets seeded, shown in the read-only row, and written to `result.json`, so the downstream field set is byte-identical to the unlocked path. Never try to "lock" a field by *omitting* `recommend.*`: the Guarantee above then falls back to the first catalog option, which silently discards the template's own value — strictly worse than asking. (The page defends against exactly that: a `locked` entry with no `recommend.*` behind it is **ignored** and the field renders as a normal picker, so a malformed lock degrades to a question rather than pinning the user to a default nobody chose.) And never lock what the template does not actually constrain; on the free-design path omit `locked` entirely so every anchor stays a real choice.
