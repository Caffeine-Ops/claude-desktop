...
# ppt-creator Python bootstrap — macOS / Linux.
#
...
if [ -x "$__ppt_py" ] && [ -f "$PPT_MASTER_VENV_DIR/.deps-ok" ]; then
  ...
  echo "[ppt-creator] Python 就绪：$PPT_PY"
...
if [ -n "$PPT_MASTER_PYTHON_HOME" ]; then
  ...
  [ -z "$__ppt_base" ] && echo "[ppt-creator] 警告：PPT_MASTER_PYTHON_HOME=$PPT_MASTER_PYTHON_HOME 下没找到解释器，回退系统 python"
fi
...
if [ -z "$__ppt_base" ]; then
  echo "[ppt-creator] 错误：没有可用的 Python 解释器。请安装 Python 3.12（推荐）或确保 app 自带 runtime 完整。"
...
case "$__ppt_ver" in
  3.14|3.15|3.16|3.17|3.18|3.19)
    echo "[ppt-creator] 警告：base 解释器是 Python $__ppt_ver，部分依赖可能无预编译 wheel，pip 会退化源码编译（慢/可能失败）。建议改用 Python 3.12。"
    ;;
esac

# ── 3. 建 venv（缺则建）+ pip install ─────────────────────────────────
if [ ! -x "$__ppt_py" ]; then
  echo "[ppt-creator] 用 $__ppt_base (Python $__ppt_ver) 建 venv → $PPT_MASTER_VENV_DIR"
  if ! "$__ppt_base" -m venv "$PPT_MASTER_VENV_DIR"; then
    echo "[ppt-creator] 错误：创建 venv 失败。"
    unset __ppt_py __ppt_req __ppt_base __c __ppt_ver
    return 1 2>/dev/null || exit 1
  fi
fi

echo "[ppt-creator] 安装依赖（首次约几分钟，之后秒过）…"
...
for __ppt_idx in "${__ppt_mirrors[@]}"; do
  if [ -n "$__ppt_idx" ]; then
    echo "[ppt-creator] 尝试镜像源：$__ppt_idx"
    ...
    echo "[ppt-creator] 尝试官方源：pypi.org"
    if "$__ppt_py" -m pip install -r "$__ppt_req" --timeout 30; then
      __ppt_ok=1
      break
    fi
  fi
  echo "[ppt-creator] 该源失败/超时，换下一个…"
done
...
echo "[ppt-creator] 错误：清华/阿里/官方三个源均安装失败。检查网络后重跑本脚本（venv 已建，只补依赖）。"
unset __ppt_py __ppt_req __ppt_base __c __ppt_ver __ppt_mirrors __ppt_idx __ppt_host __ppt_ok
return 1 2>/dev/null || exit 1
