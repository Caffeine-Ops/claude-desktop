...
4. **`composer/filePlaceholderPlugin.ts`** —— `ACCEPT_BY_KEYWORD` 表**最前面**加一条
   ...
   「招标文件」改动前命不中表里任何一条（`[/word|docx?|文档/i, '.doc,.docx']` 匹配的是
   「文档」二字，不是「文件」，不会误伤），落进「未命中→不限制」这一档，和「资料文件」
   同档，选择器不做任何引导（事实核查第 6 条）。这条规则补的是专属格式引导，放表首是
   防御性的：防以后有更宽的规则插进来抢跑，不是解一个现在就存在的冲突。放在「文稿组合
   映射」之前同理——那条会把 `.txt/.md` 也放进来，对招标文件是噪音。
