...
| 优化 / 改写类 | `optimize-existing` → `rewrite` / `polish-only` | `<项目>/drafts/`（2026-07-31 修，见后续设计）。**注意 `polish-only` 被独立触发时**（用户没走 `optimize-existing`，直接说「帮我润色」）**不建项目，落 `<cwd>/写作/<标题>-润色版.md`**，属下面那行的轻量快道 |
...
> 解决**：采用下面三个方向里的第 2 个（让工作流改落 `drafts/`），另加「没建项目的轻量快道
> 走单文件」的分界。**一稿多平台（`serialize.md`）仍不覆盖** —— 它产出的是多个平行的平台
> 版本，不是一篇文章的多个小节，需要的是平台切换器而非拼接纸面。下面的三方向记录保留作决策留痕。
>
> **⚠️ 这个 ✅ 曾经是假的，同日修正过一次，留痕在此**：第一版改动把 `polish-only.md` 一律
> 落 `<cwd>/写作/<标题>-润色版.md`，于是「优化已有作品 → 轻度润色」这一档**并没有修好**——
> `optimize-existing.md` 的 Step 1 无条件跑 `project_manager.py init` 打出 `WRITING_PROJECT=`
> 标记，而 `writingDocSource.ts` 的 `detectWritingSource` 里**项目模式无条件优先于单文件
> 模式**，右栏必然去扫那个空的 `<项目>/drafts/`，仍旧恒显「还没有正文」。最终修法是让
> `polish-only.md` 的 Step 0 **按进入方式分叉**：独立触发才落单文件，从 `optimize-existing.md`
> 路由进来落 `<项目>/drafts/`。教训：**「改了 skill 文档」不等于「链路通了」**——落点是否
> 生效取决于前端那条判定链，改落点前先确认判定链会走到哪一支。
