...
- **`tools/*` 与 `plugins/_official/` 是 vendored 上游（open-design）代码，看着「没人调」也不要删**：`scripts/postinstall.mjs` 的 buildTargets 刻意照抄上游清单（注释原话 “Mirrors open-design's postinstall buildTargets”），四个包 dev/pack/pr/serve 每次 install 都会构建。其中 `tools/pack` 是**上游的打包器**，本 fork 的 Electron 打包走的是另一条路（`apps/studio/scripts/prebundle-daemon.mjs` + electron-builder，见「命令」小节），所以根 package.json、CI、任何 build 脚本都不调 tools/pack——**这是预期状态，不是它作废了**。同理 `design-templates/` 与 `plugins/_official/examples/` 那 110 个同名目录内容几乎一样也是刻意的（见 prebundle-daemon.mjs 头注释）。判断某段代码死没死之前，先确认它不是 vendored 上游。
