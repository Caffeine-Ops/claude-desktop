...
- **窗口拖拽面只有一个写手（2026-07-08 收敛重构）**：根 layout 的 `.window-drag-strip`（fixed 全宽 46px 常驻）是唯一的 `app-region:drag` 声明——组件顶栏**禁止再标 drag**（它们随会话/分栏/切面重挂载，曾是「整窗拖不动 + 双击不缩放」间歇复发的根源，errors/ 里 7 条同族记录；canvas 面顶栏的 drag 是历史冗余，无冲突不必拆）。顶部 46px 带内的交互元素（含 portal 出去的全屏弹层）一律 `[-webkit-app-region:no-drag]` 挖洞，DOM 在 strip 之后即有效。完整事故链与纪律见 globals.css 的 `.window-drag-strip` 注释。
