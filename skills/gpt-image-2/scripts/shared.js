...
// Skill root (the dir holding SKILL.md), derived from this file's own
// location — scripts/ lives one level under it. Independent of cwd, so a
// `.env` shipped alongside the skill is found no matter where the caller
// runs `node scripts/...` from. claude-desktop uses this to ship the
// ENABLE_GARDEN_IMAGEGEN switch with the skill; the OPENAI_* key still comes
// from the host env (env.json → process.env), which wins over this file.
...
export async function loadAmbientEnv() {
  const places = [
    ...
    // Skill-bundled defaults — last, so cwd/home files and process.env still
    // win (readEnvFile pairs only fill keys not already in process.env).
