...
**`tests/fixtures/` 的例外规则请注意**：`.gitignore` 里 `!tests/fixtures/*.docx` /
`!tests/fixtures/*.pdf` 这条否定例外是上游刻意的设计（测试样本必须能进仓库），
本仓沿用不动——但它的代价是：放进这个目录的**任何** `.docx`/`.pdf` 都会被放行
进仓库，不分是不是合成测试样本。**不要把真实标书拷进 `tests/fixtures/` 里试跑**，
那样等于把商业敏感文件提交进仓库历史。

## 同步上游的做法

1. 拉上游新 commit，与 `06d0409` 做 diff
2. 逐条比对上表**每一条**本地改动是否受影响（条数会变，别按数字对，按表格逐行对）
