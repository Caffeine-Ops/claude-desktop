...
- Before final export:
   ...
   2. verify each check's logic in Python against the same inputs (openpyxl cannot compute the check formulas — recompute Actual vs Expected yourself and confirm the formula logic would show `OK`); clearly explain any limitation.
   3. read back key ranges of every sheet and confirm content is present and column widths fit the longest labels/values — fix any missing labels, source notes, formulas, or important outputs before returning the workbook (visual render pass only if LibreOffice is available, see SKILL.md).
