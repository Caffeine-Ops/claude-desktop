...
Launch the daemon (returns immediately, printing `started confirm UI in background: http://localhost:<port>` to stdout):

```bash
python3 ${SKILL_DIR}/scripts/confirm_ui/server.py <project_path> --daemon --no-browser
```

Then attach and block until the user confirms (beautify is single-pass, so the confirm writes `stage: final`):

```bash
python3 ${SKILL_DIR}/scripts/confirm_ui/server.py <project_path> --wait-only --stage final
```

`--no-browser` suppresses the system browser window — the Claude Desktop host detects this `confirm_ui/server.py` launch and embeds the page in an in-app 「浏览器」canvas tab automatically. Launch and wait are split so the host can read the real (possibly auto-advanced) port from the launch command's stdout the instant the daemon starts, instead of waiting out a blocking `--daemon --wait`.
