...
> **Presentation form (host-aware)**: when the host provides a structured-question tool (e.g. Claude Code's `AskUserQuestion`), prefer it over a flat markdown bundle so the user can point-click each choice instead of retyping changes as free text. Batch the eight items across ≤ 4 tool calls; surface 2–4 concrete preset choices per question with the recommended option first. See [`references/strategist.md`](references/strategist.md) §1 "Presentation form (host-aware)" for the full batching, fallback, and split-mode-hint rules.
