...
> **Existing-project SVG edits — always preview first**: whenever the user asks for ANY change to an already-generated ppt-master project's `svg_output/*.svg` — a full style pass or a single-word tweak on one page, it makes no difference — start the live preview first (`workflows/live-preview.md` Step 1, if not already running) **before** editing the SVG, so the user watches every change land live. Skipping this makes a correctly-applied edit invisible in the UI (the 「预览幻灯片」tab only exists while the preview service is detected running). Route color/font-wide changes through `${SKILL_DIR}/scripts/update_spec.py` when the change is a `spec_lock`-level palette/font swap propagated across every page; use direct SVG edits for anything finer-grained — either way, the preview must already be up first.
...
2. **Launch (returns immediately), then wait for Tier 1.** Two commands, in order. First launch the daemon — it **returns at once**, printing `started confirm UI in background: http://localhost:<port>` to stdout (the real, possibly auto-advanced port):
   ```bash
   python3 ${SKILL_DIR}/scripts/confirm_ui/server.py <project_path> --daemon --no-browser
   ```
   Then **immediately** attach and block until the user confirms Tier 1. **Long tool timeout — 600000 ms** (the ≈590 s budget):
   ```bash
   python3 ${SKILL_DIR}/scripts/confirm_ui/server.py <project_path> --wait-only --stage tier1
   ```
   Page opens at `http://localhost:5050` — the **same port as the Step 6 live preview** (they never run at once: this page shuts down at the end of Step 4). If 5050 is held, the launcher **auto-advances** (5051, …) — read the actual URL from the **launch command's stdout** and report it. `--no-browser` keeps the skill from opening a system browser window: the Claude Desktop host detects this `confirm_ui/server.py` launch and embeds the page in an in-app 「浏览器」canvas tab automatically, so the user sees it inside the app — no external Chrome window. **Why launch and wait are split**: the in-app browser tab needs the real port, and the launch command's stdout carries it the instant the daemon starts — a single blocking `--daemon --wait` would hold stdout captive until confirmation, so the host could never learn the port and the tab couldn't open before the user confirms. The page does **not** close after Tier 1: it shows a "deriving…" state and polls for Tier 2. **Launch or wait failure is non-fatal**: if it fails or times out (flask missing, port blocked, no GUI / remote / web host), do **NOT** troubleshoot — **on any non-zero exit, re-check `result.json` once** (a fresh `status: tier1-confirmed`) before dropping to the chat fallback. **On success (the wait-only exits 0 with a tier-1 result), do not pause or report — go straight to step 3 in the same turn.**
...
4. **Wait for the final confirmation** — attach to the already-running page, do **not** relaunch (same 600000 ms budget):
   ```bash
   python3 ${SKILL_DIR}/scripts/confirm_ui/server.py <project_path> --wait-only --stage final
   ```
