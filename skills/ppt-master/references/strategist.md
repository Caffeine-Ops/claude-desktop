...
#### Presentation form (host-aware)

> **Prefer structured prompting** — when the host provides a structured-question tool (e.g. Claude Code's `AskUserQuestion`), use it to present the bundled package. Plain markdown bundles force the user to retype any change as free text; structured tools let them point-click each choice in a UI panel. Markdown bundles remain the fallback for hosts without such a tool (Cursor, VS Code Copilot, generic LLM chats).

When using a structured-question tool:

1. **Batch the eight items into ≤ 4 tool calls** (most tools cap at 4 questions per call). Reasonable batching: call 1 = `a`/`b`/`d`/`f` (canvas / pages / style / icon library — all enumerable); call 2 = `e`/`g`/`h` (color scheme / typography / image strategy — each surfaced as 2–4 preset bundles).
2. **Each question lists 2–4 concrete preset choices**, not a binary "accept / modify". Place the recommended option first; describe key parameters in the option description (page-count band, HEX list, font family stack, icon library name).
3. **Use preview / side-by-side rendering when available** — for color schemes (HEX swatch block), font pairings (title × body sample), or layout skeletons (ASCII), put the comparison content in the tool's preview field so the user sees the variants side-by-side instead of as flat text.
4. **`c` Key Information** has no clean enumeration (target audience, occasion, core message are open-ended). Surface it as a short narrative paragraph **before** the tool calls; let the user accept it implicitly or override in their first reply.
5. **Do not duplicate** — once the recommendation is in the tool's option descriptions, do NOT also paste a long markdown bundle above it. Single source of truth per item.
6. **Split-mode hint line** (the 💡 line about heavy / normal generation mode in [`SKILL.md`](../SKILL.md#step-4) Step 4) is a notification, not a confirmation — output it as a single markdown line **after** the last tool call returns, not as another question.

When falling back to markdown (host has no structured-question tool):

- Keep the original "single bundled recommendation set" form; user replies in free text ("全部确认" / "字体改方案 2" / specific HEX overrides).
- Same content discipline as the structured form — recommend a default for every item, do not ask the user to invent values from scratch.

After the user confirms (either form), output one short acknowledgement line, then proceed silently to `design_spec.md` + `spec_lock.md` generation. No further confirmation prompts.
