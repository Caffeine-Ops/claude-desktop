...
B 类确定性转换有全套 pytest 守着，不强制人工验收；只抽这两条最容易翻车的：

| 检查项 | 合格 | 不合格 |
|---|---|---|
| 带表格的 Markdown 转 Word | 真正的 Word 表格（带边框、表头加粗、对齐生效） | 表格塌成一行行竖线文本 |
| 含 20 位发票号的 CSV 转 Excel | 发票号是完整文本原样；金额列能直接 `=SUM()` | 发票号变科学计数法（尾数丢失）/ 金额求和得 0 |
