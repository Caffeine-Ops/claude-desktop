...
⚠ **`md_to_docx.py` 不支持表格**（见下面 B1），Markdown 里的表格转进 Word 会
塌成一行行竖线文本（`| 项目 | 金额 |` 这种），脚本还会正常打印「已生成」、
退出码 0——看起来什么都没错，实际整张表格已经废了。发票、报表截图这类
A1 的主力场景恰恰最容易带表格，图上有表格且用户要 Word 时，**先说清这一点
让用户选**：① 就交付 Markdown；② 表格单独走 A2 出 Excel、正文部分走 Word。
...
1. 先认 **1–2 张**做样本，把你打算出的列摊给用户确认一次。默认列是
   **日期 / 票据类型 / 开票方 / 金额 / 税额 / 发票号 / 摘要 / 来源文件**，
   ...
   文件名——你这一步实际读的图是 `处理后/IMG_0012.jpg`，但要写进表格的是
   用户认得出的原名。
...
要 Word 时同 A1，走 `md_to_docx.py`——**同样要注意 A1 那条表格警告**：
长文档里如果有表格（财务附表、参数对照表），转 Word 会塌成竖线文本，
先跟用户说清楚。
