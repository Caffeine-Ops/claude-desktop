...
**配图必须独占一行。** 每一处 `![图说](路径)` 都要自己占一整行（前后各留一个空行），**不能夹在句子中间**——`他推开门，![门后的走廊](../images/a.png)灯是灭的。` 这种写法是错的，导出会当场报错中止。

为什么这条也是硬规则：整条导出链都建立在「一行一图」上，夹在句子里会同时打坏三件事——① **渲染**：公众号 HTML 与 Word 都是按「整行就是一张图」认图的，夹在句子里的图认不出来，`![门后的走廊](../images/a.png)` 会被当成普通文字**原样印在成品上**给读者看；② **编号与复制**：导出按图在正文里的出现顺序编号复制成 `01-x.png`、`02-x.png`，这份序号就是用户在公众号编辑器里手工插图的操作顺序；③ **封面识别**：插图清单靠「谁排第一」认封面（见上一条），一张夹在句子里的图照样会参与排序，可能被标成「封面（不要插进正文）」——一张本该出现在正文里的图，就这样在交付清单里被划掉了。

更糟的是三种格式坏得还不一样：纯文本导出正确地把它换成了 `［图：…］` 标记，HTML 和 Word 却把语法原样吐出来——同一份稿三种格式各说各话，你很难第一时间发现。所以 `export.py` 直接把它当**错误**拦下（报行号清单 + 退出码 1），不猜、不将就。

为什么封面那条是硬规则不是建议：导出脚本（`scripts/export.py` 的 `build_image_manifest`）生成插图清单时，**靠位置识别封面**——它把文档里第一个出现的图片引用直接标成「封面（在编辑器的封面位单独上传，不要插进正文）」，不看图说、不看内容，只看谁排第一。如果正文图排在封面前面，这张正文图会被错误标成「封面」，交付清单会告诉用户「这张图不要插进正文」——一张真正该出现在文章里的插图，就这样在导出环节被悄悄漏掉，作者也很难第一时间发现。所以封面的位置不是审美选择，是导出脚本的读取契约。
