...
> 🎓 [反幻觉] 这背后是两类模型的根本差异：生图模型是扩散模型，靠像素级的统计规律拼出图案，不理解「这几笔应该拼成一个字」这件事，所以越具体的文字要求，它越容易在细节上编造（幻觉出不存在的笔画）；mermaid 走的是「文字仍然是文字」的渲染管线，没有这层不确定性。工程上的经验法则是：**凡是结果需要精确、可核对的东西（文字、数字、结构关系），优先用能保证精确性的工具产出，而不是用生成式模型去凑一个看起来对的结果。** 想深入我可以展开。
