...
**识别不出来时，只问一个会改变诊断标准的关键问题**（如「这篇你当公众号推文、短篇故事、还是观察文章来优化？改法差很多」），**不要**启动主管线策划阶段的完整九项确认——那是从零创作的流程，这里稿子已经有了。目标平台同理：只在它**会影响诊断标准**时才问（比如要发小红书对字数/段落有硬要求）。
...
| 用户选了 | 路由到 | 带过去的已确认约束 |
|---|---|---|
| 轻度润色 | `polish-only.md` | 「只动文字，不动结构/观点/个人表达」——它的三个「不动」铁律正是这一档；**外加一条：成稿落 `<项目>/drafts/`，不走它的单文件快道落点**（理由见下方落点提醒） |
...
> **落点提醒：从本工作流路由出去的三档，成稿一律落 `<项目>/drafts/`**。本工作流的 Step 1
> 无条件建了项目、原稿存进 `<项目>/sources/`，所以三档的成稿都待在同一个项目目录下——
> 标准优化 / 深度改写经 `rewrite.md` 落 `<项目>/drafts/`；**轻度润色经 `polish-only.md`
> 也落 `<项目>/drafts/`**，走的是它 Step 0 表格里「从优化已有作品路由进来」那一行，
> **不是**它独立触发时那个 `<cwd>/写作/` 的轻量快道落点。
>
> **为什么必须这样（2026-07-31 修的真实 bug，别改回去）**：桌面端右栏的文档源判定里
> **项目模式无条件优先于单文件模式**，而 Step 1 已经打出了 `WRITING_PROJECT=` 标记，
> 右栏必然去扫 `<项目>/drafts/`。轻度润色这一档若还落 `<cwd>/写作/`，那个 `drafts/`
> 会是空的 → 用户看到的永远是「还没有正文」，稿子写好了工作区却全空。顺带的好处是
> `sources/` 里的原稿与成稿不再分居两处，可以直接对账。
>
> 只有**绕过本入口、直接说「帮我润色」**的用户才走 `polish-only.md` 的单文件快道
> ——那种情况下压根没有项目，也就没有这层冲突。详见设计文档「落点分界」一节。
...
1. 优化后的完整作品文件（**三档一律落 `<项目>/drafts/`**：轻度润色见 `polish-only.md`
   Step 0 表格「从优化已有作品路由进来」那一行，标准优化/深度改写见 `rewrite.md` Step 4）；
2. 修改摘要；
