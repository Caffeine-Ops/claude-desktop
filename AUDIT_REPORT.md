...
> ⚠️ **这是 2026-07-05 那天的快照，不是当前状态。** 其后项目至少经历了：设置页
> 迁往 chat 栈、窗口拖拽面收敛成单一写手、运行时组件（CLI 二进制 / python-runtime）
> 移出安装包改按需下载、写作技能上线（各项见 CLAUDE.md）。**下列 55 条问题
> 没有逐条复核过修复状态**——引用任何一条之前请先读当前代码确认它还在。
> 本文的价值在于「当时发现了什么、判断依据是什么」，不在于「现在还有什么」。
