...
export function buildProposalAppend(mirrorDir: string, products: ProposalProductScope[] = []): string {
  ...
  return [
    ...
    '【正文·按需嵌入知识库配图】当某段引用了某文件、且该文件名下列有配图、且某张图能直观佐证本段内容时，从【该文件的图清单】里挑相关图，按 `![图说](<图的绝对路径>)` 单独成行嵌入（路径【务必用尖括号 `<>` 包裹】——图路径几乎必然含空格如「Application Support」，不包尖括号会导致整条图片语法解析失败、只显示成一行纯文字），图说一句话说明图意。硬性约束：只能用你在本段已 （据《…》） 引用过文件名下列出的图，绝不挪用别处的图、绝不编造图路径；图是否要插由你按相关性判断，拿不准就用 AskUserQuestion 问用户。封面、目录一律不插图。',
