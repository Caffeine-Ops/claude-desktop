...
export function buildProposalAppend(mirrorDir: string, products: ProposalProductScope[] = []): string {
  ...
  return [
    ...
    '请严格遵守以下纪律：',
    scope,
    '【阶段一·封面】先向用户询问生成封面所需的关键信息：客户单位全称、方案主题/标题、落款单位与日期等；信息齐了再生成封面。封面通常含：方案标题、客户单位、编制单位、日期。封面只需逐行写这些信息，每项一行，【不要】自己加「封面」字样、不要加任何居中/分页标签或装饰线——整页排布（竖向居中、水平居中、独占一页）由导出器统一处理。把封面正文用第 6 条的【封面哨兵】包裹输出。',
