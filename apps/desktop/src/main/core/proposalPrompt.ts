...
export function buildProposalAppend(mirrorDir: string, products: ProposalProductScope[] = []): string {
  ...
  return [
    ...
    `【阶段一·封面】先用 AskUserQuestion 工具向用户询问生成封面所需的关键信息：客户单位全称、方案主题/标题、落款单位与日期等（把这几项合并进一次调用的多个问题里）；信息齐了再生成封面。封面通常含：方案标题、客户单位、编制单位、日期。封面请按这个结构逐行写：先写【方案标题】（用一级标题 # 开头），其下写【客户单位】等抬头信息，每项一行；然后单独一行写一个 --- （三个连字符）作分隔；--- 之下写【编制单位】与【日期】等落款信息，每项一行。导出器据此把标题块排在页面上中部、把落款块贴在页面底部、整页独占。除这一行 --- 外，【不要】自己加「封面」字样、不要加任何居中/分页标签或其它装饰线——其余排布（水平居中、竖向分布）由导出器统一处理。把封面正文用第 6 条的【封面哨兵】包裹输出。输出封面后，【必须】立即用 AskUserQuestion 工具发起封面确认：header 固定填「${PROPOSAL_COVER_CONFIRM_HEADER}」，第 1 个选项填「确认封面，生成目录」（这是放行项，务必排在【首位】），其后再给「我要调整封面」等修改类候选。用户点第 1 个选项你才进入阶段二；点修改类则按其意见改封面、用【封面哨兵】重出后再次确认。【最高优先·硬纪律】「封面确认」只能在封面哨兵块已经【完整输出到消息里】之后发起——绝不能只在确认卡的问题里口头描述封面（把标题/落款复述一遍不算交付），哨兵块没输出就发确认等于什么都没写。（系统设有硬门：右侧文档没收到封面就发起封面确认，会被自动打回、要求先补写封面哨兵块再重新发起确认。）`,
