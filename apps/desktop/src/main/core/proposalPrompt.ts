...
export function buildProposalAppend(mirrorDir: string, products: ProposalProductScope[] = []): string {
  ...
  return [
    ...
    '请严格遵守以下纪律：',
    scope,
    '【阶段一·封面】先向用户询问生成封面所需的关键信息：客户单位全称、方案主题/标题、落款单位与日期等；信息齐了再生成封面。封面通常含：方案标题、客户单位、编制单位、日期。把封面正文用第 6 条的【封面哨兵】包裹输出。',
    '【阶段二·目录】收到「确认封面」推进消息后，参考该产品在知识库里的资料结构与售前建设方案的常见章节（如：项目背景、需求分析、建设目标、总体方案设计、功能详述、实施计划、售后服务等），提出一份【章节目录大纲】（用有序列表逐章列出），用第 6 条的【目录哨兵】包裹。用户可能直接编辑目录，或用自然语言要你增删/调整章节——按用户修订重新输出目录，不自行发挥。',
    '【阶段三·正文】收到「确认目录，开始正文」推进消息后（消息里会带上已确认的目录），严格【按该目录逐章撰写正文】：章节标题与顺序以目录为准，不自行增删章节。一次聚焦一章，每章用第 6 条的【正文哨兵】包裹；原文清晰、足以直接组织时可直接起草，不必逐段确认；只有该章关键要点确实不明确时，才先问用户再起草。用户标注「一条条介绍」「逐条」的部分要逐条列举（每条：小标题 + 该条内容 + 来源），不要并成一段。',
