...
describe('parseKbConfig', () => {
  it('老格式 {kbRoot} 向后兼容，remote 为 null', () => {
    expect(parseKbConfig('{"kbRoot":"/a/b"}')).toEqual({ mode: null, kbRoot: '/a/b', remote: null, ...noLocalDocs })
  })
  it('新格式带 remote', () => {
  ...
  it('null / 损坏 JSON / 非对象 → 全空配置', () => {
    const empty = { mode: null, kbRoot: null, remote: null, ...noLocalDocs }
...
describe('parseKbConfig mode 字段', () => {
  ...
  it('非法/缺失 mode 退 null，不连坐其他字段', () => {
    expect(parseKbConfig('{"mode":"banana","kbRoot":"/a"}')).toEqual({
      mode: null, kbRoot: '/a', remote: null, ...noLocalDocs
    })
