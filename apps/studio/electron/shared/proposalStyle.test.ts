...
// Bug 2：切模板智能合并——保留用户微调，只换走「没动过」的字段。
describe('mergeTemplateSwitch（切模板保留微调）', () => {
  it('未微调切模板 → 等价于整份采用新模板', () => {
    const from = cloneProposalStyle('classic')
    const out = mergeTemplateSwitch(from, 'business')
    expect(out).toEqual(PROPOSAL_TEMPLATES.business)
  })

  it('微调过的字段保留，未动的字段跟随新模板', () => {
    const from = cloneProposalStyle('classic')
    from.body.font = '楷体' // classic 默认 body.font=宋体 → 用户动过
    from.lineMultiple = 2.0 // classic 默认 1.65 → 用户动过
    const out = mergeTemplateSwitch(from, 'business')
    // 保留用户微调
    expect(out.body.font).toBe('楷体')
    expect(out.lineMultiple).toBe(2.0)
    // 未动的字段跟随 business
    expect(out.templateKey).toBe('business')
    expect(out.body.size).toBe(PROPOSAL_TEMPLATES.business.body.size)
    expect(out.margin).toBe(PROPOSAL_TEMPLATES.business.margin)
    expect(out.h1.color).toBe(PROPOSAL_TEMPLATES.business.h1.color) // 采用 business 的 ACCENT
  })

  it('点当前已选中的模板卡 = no-op（绝不清空微调）', () => {
    const from = cloneProposalStyle('classic')
    from.body.font = '楷体'
    from.spaceAfterPt = 12
    const out = mergeTemplateSwitch(from, 'classic')
    expect(out).toEqual(from)
  })

  it('多次切换微调持续累积（classic→business→academic 保留早先微调）', () => {
    const from = cloneProposalStyle('classic')
    from.body.font = '楷体'
    const a = mergeTemplateSwitch(from, 'business') // body.font 楷体 保留
    const b = mergeTemplateSwitch(a, 'academic') // 仍保留
    expect(b.body.font).toBe('楷体')
    expect(b.templateKey).toBe('academic')
    // 未动的 margin 一路跟随到 academic
    expect(b.margin).toBe(PROPOSAL_TEMPLATES.academic.margin)
  })

  it('用户设了标题色后切到无色模板 → 保留用户色', () => {
    const from = cloneProposalStyle('business') // h1.color = ACCENT
    from.h1.color = 'ff8800' // 用户改成橙色
    const out = mergeTemplateSwitch(from, 'classic') // classic h1 无 color
    expect(out.h1.color).toBe('ff8800') // 保留用户色，不被 classic 的「无色」抹掉
  })
})
