...
export interface WritingFileMeta {
  ...
  size: number
  /**
   * 内容的 sha1 十六进制摘要（`scanWritingDoc` 用 `node:crypto` 现读现算，见该函数注释）。
   * **轮询判定「文件变没变」的签名要素**——历史上这里放的是纳秒精度的 mtime
   * （`statSync(p, { bigint: true }).mtimeNs`），当时的论断是「纳秒精度下『等长改写 +
   * 同一纳秒写入』不可能同时成立，缺口被彻底堵死」。**这个论断已被 Windows 上的真实失败
   * 证伪**：CI 的 Windows 腿上，两次连续的真实写入（无 mock）拿到完全相同的 mtimeNs——
   * Windows 的文件时间源粒度远粗于纳秒，「不可能同时成立」并不成立。后果是 Windows 上
   * 「等长改写」（中文里「的→地」「，→、」这类单字符替换很常见）会让 `name:mtimeNs:size`
   * 三要素全部不变，轮询判定「未变化」而跳过拉正文——静默漏刷，界面停在旧内容且不报错，
   * 那个被认为堵死的洞原样存在。
   *
   * 改用内容哈希后不存在这个精度问题：只要内容有哪怕一个字符不同，哈希必然不同（哈希碰撞
   * 概率对本场景可忽略）。当初否决内容哈希的理由是「每轮要读全部文件内容，等于把『scan
   * 只回元信息、正文按需拉』的设计意义清空」——这个担忧被实测大幅缓解，但**实测有明确前提，
   * 不能读成"任何情况下都可忽略"**（这正是本次要修的 bug 的同一种错误——把一个有前提的
   * 观察当成普适结论）：40 节 / 820KB / 28 万字的长篇规模，**本地磁盘、系统页缓存已预热**
   * 条件下，本机实测纯 stat 每轮约 0.06ms，stat+读全文+sha1 每轮约 0.76ms，差值约 0.7ms，
   * 相对 2000ms 的轮询间隔占比约 0.035%。
   *
   * **已知限制（未覆盖场景，不是"已解决"）**：`scanWritingDoc` 挂在
   * `ipcMain.handle(WRITING_SCAN, ...)` 下，handler 虽标 async 但函数体是纯同步
   * `statSync`/`readFileSync`——阻塞的是 Electron 主进程唯一的事件循环，卡住的不只是
   * 写作面板所在窗口，是全部窗口的 IPC。`writingDocSource.ts` 顶注明确写着 UNC 网络共享
   * 路径「是真实构建目标，不是假设性场景」——项目目录若挂在慢速网络盘上，或者节文件被
   * 写成远超 820KB 单节（未预热/冷读场景），`readFileSync` 的延迟可能是本地 SSD 实测的
   * 数量级差异，届时这次改动新增的每节一次 readFileSync 会比改动前"只 stat"更容易让这次
   * 阻塞变得可感知。这不是本次改动引入的新架构问题（scan 在改动前就是同步实现），只是
   * 内容哈希把"读多少字节"从 0 变成了"整份正文"，放大了原有同步阻塞的量级。若未来要处理，
   * 方向是把 scanWritingDoc 的 IPC handler 真正异步化（用 `fs/promises` 或挪到 worker），
   * 但那是独立的改动，不在本次修复范围内。
   */
  contentHash: string
}
