...
export interface WritingFileMeta {
   ...
   * 只回元信息、正文按需拉』的设计意义清空」——这个担忧已被实测证伪：40 节 / 820KB / 28 万字
   * 的长篇规模下，本机实测纯 stat 每轮约 0.06ms，stat+读全文+sha1 每轮约 0.76ms，差值约
   * 0.7ms，相对 2000ms 的轮询间隔占比约 0.035%，可忽略不计，且轮询本身每 2 秒一次，文件
   * 必然在系统页缓存里。
   */
  contentHash: string
}
