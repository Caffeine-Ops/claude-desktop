...
describe('buildKbIndex', () => {
  test('全量构建：v3 索引 + 镜像 + importedAtMs/sizeBytes', async () => {
    ...
    const idx = await buildKbIndex({ kbRoot: root, outDir: out, now: 1000 })
  ...
  test('增量：未变文件跳过且 importedAtMs 保留；删除的文件从索引消失', async () => {
    const { root, out } = fixture()
    await buildKbIndex({ kbRoot: root, outDir: out, now: 1000 })
    writeFileSync(join(root, '智慧水务', '平台A', '新增.txt'), '第二篇', 'utf8')
    const idx2 = await buildKbIndex({ kbRoot: root, outDir: out, now: 2000 })
    ...
    const idx3 = await buildKbIndex({ kbRoot: root, outDir: out, now: 3000 })
  ...
  test('同路径内容变更（sha1 变）重转：importedAtMs 保留旧值、镜像已更新', async () => {
    ...
    const idx1 = await buildKbIndex({ kbRoot: root, outDir: out, now: 1000 })
