  ...
  void runKbSync({
    ...
    .then(
      (final) => {
        ...
        // 2026-09-24：这里原本在同步成功后 resetEmbedWorker()——杀掉端着旧向量内存表的
        // embedding worker，让下次检索重 fork 校验 fingerprint。向量化栈删除后检索每次
        // 都现读镜像文件（BM25 无常驻内存表、无 fingerprint），同步完成后无状态需要失效。
      },
