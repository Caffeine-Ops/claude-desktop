 ...
 * useWritingPoll 每几秒重扫（比对 name:contentHash:size 签名）。没有事件，只有轮询。
 * 这带来两个必须显式解决的问题：
 *
...
// 毫秒签名会与上一轮撞车，判定『未变化』而漏刷」。`WritingSection`（读正文用的类型）只带
// 毫秒精度的 `mtimeMs`，没有内容哈希字段——用它当稳定判据的签名，等长替换（中文里
// 「的→地」「，→、」这类单字符替换很常见）若又恰好落在同一毫秒内，会被误判「未变化」，
// 在文件其实还在被写的那一帧上发起出图（半截指令）。这正是稳定判据存在的唯一理由，不能被
// 它自己依赖的精度问题绕过去。改用内容哈希后不存在这个精度问题：只要内容有哪怕一个字符
// 不同，哈希必然不同（哈希碰撞概率对本场景可忽略），且 `sections` 已经在内存里，不需要
// 额外读盘。
//
// 【与轮询外层签名（stores/writing.ts）的关系】外层签名后来也从纳秒 mtime 改成了
// `WritingFileMeta.contentHash`（scan 阶段由 main 进程用 sha1 现算，同一次证伪：纳秒精度
// 在 Windows 上不成立）——两处签名各自独立计算内容哈希，不是同一次哈希结果的复用：外层
// 哈希的是磁盘文件的字节内容（scan 只回元信息，供轮询判定「要不要拉正文」），这里的
// `genImageRawHash` 哈希的是已经拉到内存里的 `sec.markdown`（供判定「拉到手的正文连续
// 两轮是否稳定」）。两层判据关心的问题不同，没有必要也没有办法共用一次计算结果。
//
