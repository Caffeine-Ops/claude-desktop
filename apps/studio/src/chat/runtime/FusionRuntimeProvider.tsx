...
export function FusionRuntimeProvider({
  ...
  const runtime = useExternalStoreRuntime({
    ...
    onNew: async (message: AppendMessage) => {
      ...
      // 消息以 ppt-creator 斜杠开头（EmptyState ScenarioRail 的「制作PPT」
      // chip、SkillPicker、或手敲 `/`，殊途同归都是 leading 命令）就把本会话
      // 标记为 slides 会话 → ThreadView 双分栏工作台。旧机制是「发送时全局
      // mode===slides 则 markIfSlides + 拼 /ppt-creator 前缀」，模式入口收敛到
      ...
      // 场），会话中途才第一次带上 ppt-creator 命令同样应该点亮两栏工作台，
      // 不该被"必须是第 1 条消息"卡住。
      // 四种形态都要认：`cowork:` 是 2026-07-29 迁成按需下载插件后的正式命名
      // 空间，裸名覆盖用户自装副本，`claude-desktop:ppt-master` 与裸 `ppt-master`
      // 是历史会话里的旧命令——认漏旧名会让老的 PPT 会话重开后不再点亮双栏
      // 工作台（用户视角就是「我的历史记录坏了」）。
      const alreadyPptSlash = /^\/(cowork:|claude-desktop:)?ppt-(creator|master)\b/.test(baseText)
