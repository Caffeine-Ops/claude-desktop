...
const STRINGS = {
  zh: {
    ...
    kbPreview: '预览',
    kbRetry: '重试',
    kbToolingTitle: '需要安装文档转换插件 markitdown',
    kbToolingDesc: '知识库靠它把 Word / PPT / Excel 转成可检索的文本。它是本机命令行工具，换电脑需重新安装一次。',
    kbToolingInstall: '一键安装',
    kbToolingInstalling: '正在安装…（可能需要一两分钟，请勿关闭）',
    kbToolingManual: '手动安装命令',
    kbToolingUnsupported: '未找到 Python 或 pipx，无法自动安装。请先安装 Python（python.org），再点「一键安装」；或用下方命令手动安装。',
    kbToolingFailed: '安装失败。可展开日志排查，或用下方命令手动安装后重启应用。',
    kbToolingLog: '安装日志',
    kbBuilding: '正在构建索引',
  ...
  en: {
    ...
    kbPreview: 'Preview',
    kbRetry: 'Retry',
    kbToolingTitle: 'Install the document converter “markitdown”',
    kbToolingDesc: 'The KB uses it to turn Word / PPT / Excel into searchable text. It is a local CLI tool — reinstall it once per machine.',
    kbToolingInstall: 'Install now',
    kbToolingInstalling: 'Installing… (may take a minute or two — don’t close)',
    kbToolingManual: 'Manual install command',
    kbToolingUnsupported: 'Python / pipx not found — cannot auto-install. Install Python (python.org) first, then click Install now; or use the command below.',
    kbToolingFailed: 'Install failed. Expand the log to diagnose, or install manually with the command below and restart the app.',
    kbToolingLog: 'Install log',
