 ...
 *     被实测大幅缓解，但**实测有明确前提，不是普适结论**——40 节 / 820KB / 28 万字规模，
 *     **本地磁盘、系统页缓存已预热**条件下，本机实测纯 stat 每轮约 0.06ms，stat+读全文+sha1
 *     每轮约 0.76ms，差值约 0.7ms，占 2000ms 轮询间隔约 0.035%。**已知限制**：这次改动没有
 *     把 scanWritingDoc 从同步 IPC handler 改成异步（那是独立的改动，见该函数所在文件顶注），
 *     所以 readFileSync 的耗时仍会阻塞 Electron 主进程的事件循环；见下方第 3 点已经指出的
 *     UNC 网络共享盘场景——原本只 stat 时这个风险就存在，现在每节多了一次整份读取，网络盘
 *     或超大节文件上这次改动会放大而不是消除这个已有的同步阻塞风险。
 *
