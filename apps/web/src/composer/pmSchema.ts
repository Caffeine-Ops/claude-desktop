...
/**
 * ProseMirror schema for the web chat composer — ported from the desktop
 * app (`apps/desktop/src/renderer/src/composer/pmSchema.ts`) with no
 * changes to the wire format. Deliberately minimal: doc / paragraph /
 * text plus two inline *atom* nodes (`slash`, `mention`). **No marks.**
 *
 * Why atom nodes for slash / mention instead of plain text + an overlay
 * (web's old approach)?
 *
 *   - An atom node is rendered by a NodeView (see `chipNodeView.ts`) as a
 *     single, indivisible pill. The browser draws the caret at real DOM
 *     boundaries between nodes, so we no longer measure pixel positions
 *     or keep a transparent textarea + absolutely-positioned overlay in
 *     sync (the source of the highlight-drift bugs this migration fixes).
 ...
 * textarea held (`/cmd`, `@path`), because the daemon parses that raw
 * text downstream. The chip is purely a *visual* layer; the wire format
 * is unchanged — submitted prompt stays byte-for-byte identical.
 */
export const composerSchema = new Schema({
  nodes: {
    ...
    paragraph: {
      ...
      // models those as paragraph splits.
      parseDOM: [{ tag: 'p' }],
      toDOM: () => ['p', 0],
    },
    text: { group: 'inline' },
    ...
    // ---- file/context mention atom (e.g. `@src/foo.ts` or `@"a b.ts"`)
    mention: {
      group: 'inline',
      inline: true,
      atom: true,
  ...
  // marks: intentionally omitted — the composer submits a plain string.
});
 ...
 *
 * This is the single source of truth for "what the daemon receives".
 * It MUST stay byte-for-byte compatible with what the user typed.
 */
 ...
 * tokens so an externally-set draft (e.g. EmptyState prefill, a draft
 * restore, a slash-pick rewrite, or send clearing it to '') renders with
 * chips. Each text line becomes one paragraph; tokens become atom nodes.
 *
 * NOTE: web's mention tokens can contain spaces (e.g. `@Slack MCP`,
 * `@Deck Builder`) because they're labels, not file paths. The bare
 * tokenizer below only lights up whitespace-free `@`/`/` runs (matching
 * the suggestion-plugin trigger). Multi-word labels are reconstructed as
 * chips by the caller via `parseTextWithEntities` when the entity list
 * is known; the plain `parseText` keeps them as text (still serializes
 * losslessly — the chip is cosmetic).
 */
